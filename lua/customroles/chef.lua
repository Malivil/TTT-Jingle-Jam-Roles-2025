...
local burger_time = CreateConVar("ttt_chef_burger_time", "60", FCVAR_REPLICATED, "The amount of time the burger effect should last", 1, 120)
...
local hotdog_time = CreateConVar("ttt_chef_hotdog_time", "60", FCVAR_REPLICATED, "The amount of time the hot dog effect should last", 1, 120)
...
local fish_time = CreateConVar("ttt_chef_fish_time", "60", FCVAR_REPLICATED, "The amount of time the fish effect should last", 1, 120)
...
local burnt_time = CreateConVar("ttt_chef_burnt_time", "60", FCVAR_REPLICATED, "The amount of time the burnt food effect should last", 1, 120)
...
CreateConVar("ttt_chef_shop_sync", "0", FCVAR_REPLICATED)
CreateConVar("ttt_chef_shop_random_percent", "0", FCVAR_REPLICATED, "The percent chance that a weapon in the shop will not be shown for the Chef", 0, 100)
CreateConVar("ttt_chef_shop_random_enabled", "0", FCVAR_REPLICATED, "Whether shop randomization should run for the Chef")
...
if CLIENT then
    ...
    AddHook("TTTTutorialRoleText", "Chef_TTTTutorialRoleText", function(role, titleLabel)
        if role == ROLE_CHEF then
            local html = "The " .. ROLE_STRINGS[ROLE_CHEF] .. " is "
            local roleColor
